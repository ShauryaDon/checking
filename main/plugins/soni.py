...
async def run_batch(userbot, client, sender, countdown, link):
    for i in range(len(ids)):
        ...
        try: 
            ...
            if countdown.text != count_down:
                await countdown.edit(count_down, buttons=[[Button.url("Join Channel", url="https://t.me/alphajesh")]])
        n = i + 1
        if n == len(ids):
            return -2

C = "/cancel"
START_PIC = "https://graph.org/Om-Namah-Shiway-03-23"
TEXT = "👋 Hi, This is 'Paid Restricted Content Saver' bot Made with ❤️ by __**Shaurya Soni**__."
...
async def start_command(event):
    ...
    buttons = [
        [Button.inline("Cancel", data="cancel"),
         Button.inline("Cancel", data="cancel")],
        [Button.url("Join Channel", url="https://telegram.dog/alphajesh")]
    ]
